...
class ChallongeSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [f"https://challonge.com/madness2022/predictions?page={i}" for i in range(1,5)]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response, **kwargs):
        for prediction_link in response.css('a::attr(href)').getall():
            if 'https://challonge.com/tournaments/10777511/predictions' in prediction_link:
    ...
    def parse_prediction(self, response):
        ...
        with open('/home/deadhand/Documents/filmspotting-2019/results/2022_predictions.csv', 'a') as results:
